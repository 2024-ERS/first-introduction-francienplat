...
vdat1<-vdat|>
  ...
  dplyr::filter(!Species_ID %in% c("bare", "litter","mosses","SalicEur","SalicPro")) #exclude these Salic combin
...
vdat2<-vdat1|>
  ...
  dplyr::summarize(sumcov=sum(cover,na.rm=T))|>
...
vdat3|>dplyr::filter(rank<=10)|>
  ggplot(aes(x=as.factor(TransectPoint_ID),y=factor(-year),fill=cover))+
  geom_tile() + 
